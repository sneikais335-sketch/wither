...
TRANSLATIONS = {
    ...
    "KG": {
        ...
        "settings_sub": "Өзүңүзгө ылайыктаңыз",
        "display": "Дисплей",
...
class WeatherApp(ctk.CTk):
    def __init__(self):
        ...
        # Setup main container for pages
        self.main_container = ctk.CTkFrame(self, fg_color="transparent")
        self.main_container.grid(row=0, column=0, sticky="nsew")
        
        # Sidebar calendar (hidden initially)
    ...
    def update_popular_cities(self):
        if not hasattr(self, 'pop_frame') or not self.pop_frame.winfo_exists():
            return
            
        for child in self.pop_frame.winfo_children():
            child.destroy()
        ...
        if prefix:
            ctk.CTkLabel(self.pop_frame, text=prefix, font=(self.font_family, 11), text_color="#6E7191").pack(side="left", padx=(0, 5))
            
        cities = [c.strip() for c in cities_str.split("  ") if c.strip()]
        
        for city in cities:
            btn = ctk.CTkButton(self.pop_frame, text=city, font=(self.font_family, 11, "underline"), fg_color="transparent", 
                                text_color="#3B82F6", hover_color="#E8EEF4", height=20, width=0,
                                cursor="hand2", command=lambda c=city: self.fetch_weather(c))
            btn.pack(side="left", padx=2)

    # ==========================================
    # UTILITIES (Gradients & Custom Shapes)
    # ==========================================
    ...
    def create_rounded_gradient_bar(self, parent, width, height, color1, color2):
        ...
        for y in range(height*2):
            ...
            g = int(g1 + (g2 - g1) * ratio)
            b = int(b1 + (b2 - b1) * ratio)
            grad_draw.line([(0, y), (width*2, y)], fill=(r, g, b, 255))
        ...
        img = ctk.CTkImage(light_image=grad, size=(width, height))
    ...
    def clear_screen(self):
        for widget in self.main_container.winfo_children():
            widget.destroy()
        # Filter out dynamic labels that were destroyed
    ...
    def create_menu_btn(self, parent, icon_char, icon_color, text_key, command):
        ...
        icon_lbl = ctk.CTkLabel(btn_frame, text=icon_char, font=(self.font_family, 18), 
                                text_color="#FFFFFF", fg_color=icon_color, width=36, height=36, corner_radius=10)
    ...
    def set_language(self, lang):
        ...
        if hasattr(self, 'current_city') and getattr(self, 'current_city', None):
    ...
    def create_calendar_widget(self):
        ...
        ctk.CTkButton(header_row, text="<", font=(self.font_family, 14), width=25, height=25, 
                      fg_color="transparent", text_color="#A0AEC0", hover_color="#F0F0F0").pack(side="left")
                      
        ctk.CTkLabel(header_row, text=now.strftime("%B %Y"), font=(self.font_family, 14, "bold"), 
                     text_color="#4B5563").pack(side="left", expand=True)
                     
        ctk.CTkButton(header_row, text=">", font=(self.font_family, 14), width=25, height=25, 
                      fg_color="transparent", text_color="#A0AEC0", hover_color="#F0F0F0").pack(side="right")
        ...
        for idx, h in enumerate(headers):
            ctk.CTkLabel(days_grid, text=h, font=(self.font_family, 10, "bold"), 
                         text_color="#D1D5DB").grid(row=0, column=idx, pady=(0, 15))
        ...
        for r in range(1, 6):
            for c in range(7):
                if day_counter <= 31:
                    btn = ctk.CTkButton(days_grid, text=str(day_counter), 
    ...
    def select_date(self, selected_btn, day):
        for item in self.day_buttons:
            btn = item["widget"]
            if btn == selected_btn:
                btn.configure(fg_color="#10B981", text_color="#FFFFFF", font=(self.font_family, 12, "bold"), hover_color="#059669")
            else:
                btn.configure(fg_color="transparent", text_color="#4B5563", font=(self.font_family, 12, "normal"), hover_color="#E5E7EB")
    ...
    def toggle_calendar(self):
        if self.calendar_visible:
            ...
            self.sidebar_calendar.place(relx=1.0, y=95, x=-30, anchor="ne")
    ...
    def build_header(self, parent, is_main=False, title_text=""):
        ...
        if is_main:
            burger_btn = ctk.CTkButton(header_frame, text="☰", font=(self.font_family, 18), width=45, height=45, 
                                       fg_color="#FFFFFF", text_color="#14142B", corner_radius=12, hover_color="#F0F0F0",
            ...
            icon_lbl = ctk.CTkLabel(date_container, text="🗓", font=(self.font_family, 26), text_color="#14142B")
            ...
            self.header_city_lbl = ctk.CTkLabel(text_frame, text="Bishkek", font=(self.font_family, 13), text_color="#6E7191")
            self.header_city_lbl.pack(anchor="w", pady=0)
            
            date_str = datetime.datetime.now().strftime("%d.%m.%Y")
            date_lbl = ctk.CTkLabel(text_frame, text=date_str, font=(self.font_family, 16, "bold"), text_color="#14142B")
            date_lbl.pack(anchor="w", pady=0)
            ...
            date_lbl.bind("<Button-1>", lambda e: self.toggle_calendar())
    ...
    def show_main_screen(self):
        ...
        self.search_entry = ctk.CTkEntry(search_frame, height=45, corner_radius=25, fg_color="#FFFFFF", border_color="#E2E8F0", border_width=1,
                                         font=(self.font_family, 13))
        self.search_entry.pack(side="left", fill="x", expand=True)
        self.search_entry.bind('<Return>', lambda e: self.perform_search())
        self.register_i18n(self.search_entry, "search_placeholder", attr="placeholder_text")
        
        search_btn = ctk.CTkButton(search_frame, width=100, height=45, corner_radius=25, 
                                   fg_color="#14142B", hover_color="#2A2A40", font=(self.font_family, 13, "bold"),
                                   command=self.perform_search)
        search_btn.pack(side="right", padx=(10, 0))
        self.register_i18n(search_btn, "search_btn")
        
        self.save_btn = ctk.CTkButton(search_frame, width=80, height=45, corner_radius=25,
                                      fg_color="#FFFFFF", border_width=1, border_color="#10B981", 
                                      text_color="#10B981", hover_color="#F0FDF4", font=(self.font_family, 13, "bold"),
        ...
        self.ui_city_lbl = ctk.CTkLabel(text_frame, text="Bishkek", font=(self.font_family, 28, "bold"), text_color="#FFFFFF")
        self.ui_city_lbl.pack(anchor="w")
        
        date_str = datetime.datetime.now().strftime("%A, %B %d, %Y")
        ctk.CTkLabel(text_frame, text=date_str, font=(self.font_family, 12), text_color="#E2E8F0").pack(anchor="w", pady=(0, 20))
        
        temp_frame = ctk.CTkFrame(text_frame, fg_color="transparent")
        temp_frame.pack(anchor="w")
        self.ui_temp_lbl = ctk.CTkLabel(temp_frame, text="18", font=(self.font_family, 64, "bold"), text_color="#FFFFFF")
        self.ui_temp_lbl.pack(side="left")
        ctk.CTkLabel(temp_frame, text="°C", font=(self.font_family, 32, "bold"), text_color="#FFFFFF").pack(side="left", anchor="n", pady=(10,0))
        
        self.ui_desc_lbl = ctk.CTkLabel(text_frame, text="Partly Cloudy", font=(self.font_family, 16), text_color="#FFFFFF")
        self.ui_desc_lbl.pack(anchor="w")
        
        self.ui_feels_lbl = ctk.CTkLabel(text_frame, text="Feels like 16°C", font=(self.font_family, 12), text_color="#CBD5E1")
        ...
        metrics = [
            {"t_key": "humidity", "v": "65%", "c": "#3B82F6", "i": "💧"},
            {"t_key": "wind_speed", "v": "15 km/h", "c": "#10B981", "i": "💨"},
            {"t_key": "visibility", "v": "10 km", "c": "#8B5CF6", "i": "👁"},
            {"t_key": "pressure", "v": "1013 mb", "c": "#F59E0B", "i": "⏱"},
            {"t_key": "dew_point", "v": "12°C", "c": "#EC4899", "i": "🌡"},
            {"t_key": "uv_index", "v": "6", "c": "#EF4444", "i": "☀"}
        ]
        ...
        for idx, m in enumerate(metrics):
            ...
            ctk.CTkLabel(icon_bg, text=m["i"], text_color="#FFFFFF", font=(self.font_family, 16)).place(relx=0.5, rely=0.5, anchor="center")
        ...
        chart_card = ctk.CTkFrame(content_wrapper, fg_color="#FFFFFF", corner_radius=20, height=250)
        ...
        chart_scroll = ctk.CTkScrollableFrame(chart_card, fg_color="transparent", orientation="horizontal", height=160)
        chart_scroll.pack(fill="x", padx=10, pady=(0, 10))
        
        hourly = [("12°", "☁", "0:00", 30), ("11°", "☁", "1:00", 25), ("11°", "☁", "2:00", 25), 
                  ("10°", "☁", "3:00", 20), ("10°", "☁", "4:00", 20), ("11°", "☁", "5:00", 25),
                  ("12°", "☁", "6:00", 30), ("13°", "☁", "7:00", 35), ("14°", "☁", "8:00", 40),
                  ("16°", "☁", "9:00", 50), ("17°", "☼", "10:00", 60), ("18°", "☼", "11:00", 70),
                  ("20°", "☼", "12:00", 85), ("21°", "☼", "13:00", 95), ("22°", "☼", "14:00", 100),
                  ("23°", "☼", "15:00", 110), ("22°", "☼", "16:00", 100)]
                  
        for h_temp, h_icon, h_time, h_height in hourly:
            col = ctk.CTkFrame(chart_scroll, fg_color="transparent", width=55)
            ...
            ctk.CTkLabel(col, text=h_time, font=(self.font_family, 10), text_color="#A0AEC0").pack(side="top")
        ...
        days = [("Monday", "☼", "Sunny", "15°", "25°", "#FBBF24", "#F59E0B", "#EAB308"),
                ("Tuesday", "☁", "Partly Cloudy", "14°", "22°", "#60A5FA", "#FBBF24", "#6E7191"),
                ("Wednesday", "☁", "Cloudy", "12°", "20°", "#9CA3AF", "#D1D5DB", "#6E7191"),
                ("Thursday", "🌧", "Rainy", "10°", "18°", "#3B82F6", "#60A5FA", "#3B82F6"),
                ("Friday", "☁", "Partly Cloudy", "13°", "21°", "#60A5FA", "#FBBF24", "#6E7191"),
                ("Saturday", "☼", "Sunny", "14°", "24°", "#FBBF24", "#F59E0B", "#EAB308"),
                ("Sunday", "☼", "Sunny", "16°", "26°", "#FBBF24", "#F59E0B", "#EAB308")]
                
        for d_name, d_ico, d_desc, d_min, d_max, c1, c2, ico_col in days:
            row = ctk.CTkFrame(forecast_card, fg_color="transparent", height=45)
            row.pack(fill="x", padx=25)
            row.pack_propagate(False)
            
            ctk.CTkLabel(row, text=d_name, font=(self.font_family, 14), text_color="#14142B", width=120, anchor="w").pack(side="left")
            
            ico_lbl = ctk.CTkLabel(row, text=d_ico, font=(self.font_family, 18), text_color=ico_col, width=30, anchor="w")
            ico_lbl.pack(side="left")
            
            desc_lbl = ctk.CTkLabel(row, text=d_desc, font=(self.font_family, 14), text_color="#6E7191", width=150, anchor="w")
            desc_lbl.pack(side="left")
            
            ctk.CTkLabel(row, text=d_max, font=(self.font_family, 14, "bold"), text_color="#14142B", anchor="e", width=40).pack(side="right")
            bar_lbl = self.create_rounded_gradient_bar(row, 100, 6, c1, c2)
            bar_lbl.pack(side="right", padx=15, pady=20)
            ctk.CTkLabel(row, text=d_min, font=(self.font_family, 14), text_color="#A0AEC0", anchor="w", width=40).pack(side="right")
    ...
    def save_current_city(self):
        ...
        for c in self.saved_cities:
            if c["name"] == self.current_city_data["name"]:
    ...
    def fetch_weather(self, city_name):
        ...
        if OPENWEATHER_API_KEY == "YOUR_API_KEY_HERE" or not OPENWEATHER_API_KEY:
            self.ui_desc_lbl.configure(text="API Key Missing!")
            return
        ...
        try:
            ...
            if not geo_data:
                self.ui_desc_lbl.configure(text="City not found")
                return
            ...
            weather_url = f"https://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&appid={OPENWEATHER_API_KEY}&units={units}&lang={lang_param}"
            w_req = urllib.request.urlopen(weather_url)
            w_data = json.loads(w_req.read())
            
            self.update_weather_ui_live(name, w_data)
        except Exception as e:
            print("Error fetching weather:", e)
            self.ui_desc_lbl.configure(text="Network Error")

    def update_weather_ui_live(self, city_name, current_data):
        self.ui_city_lbl.configure(text=city_name)
        if hasattr(self, 'cal_city_lbl'):
            self.cal_city_lbl.configure(text=city_name)
        if hasattr(self, 'header_city_lbl'):
            self.header_city_lbl.configure(text=city_name)
        ...
        self.current_city_data = {
            ...
            "humidity": f"{humidity}%",
            "wind": f"{wind_speed} {unit_str}"
        }

    # ==========================================
    # SCREEN 2: SAVED CITIES
    # ==========================================
    def show_saved_cities_screen(self):
        ...
        export_btn = ctk.CTkButton(btn_frame, font=(self.font_family, 13, "bold"), 
                                   fg_color="#10B981", hover_color="#059669", height=40, corner_radius=10, width=180)
        ...
        if not cities:
            empty_lbl = ctk.CTkLabel(grid_frame, text="No saved cities yet.", font=(self.font_family, 16), text_color="#A0AEC0")
        ...
        for idx, city in enumerate(cities):
            ...
            # Use solid hex colors instead of rgba to avoid rendering crashes
            if idx % 2 == 0:
                card_bg = "#314BF5"
                element_bg = "#4B63F6"
                hover_bg = "#5B73F6"
            ...
            ctk.CTkLabel(title_box, text=city["name"], font=(self.font_family, 24, "bold"), text_color="#FFFFFF").pack(anchor="w")
            ctk.CTkLabel(title_box, text=city["date"], font=(self.font_family, 12), text_color="#D1D5DB").pack(anchor="w")
            
            btn_box = ctk.CTkFrame(top_row, fg_color="transparent")
            btn_box.pack(side="right", anchor="n")
            
            ctk.CTkButton(btn_box, text="⬇", width=35, height=35, corner_radius=10, fg_color=element_bg, hover_color=hover_bg, text_color="#FFFFFF", font=("", 16)).pack(side="left", padx=8)
            
            def del_city(name=city["name"]):
                self.saved_cities = [c for c in self.saved_cities if c["name"] != name]
                self.show_saved_cities_screen()
                
            ctk.CTkButton(btn_box, text="🗑", width=35, height=35, corner_radius=10, fg_color=element_bg, hover_color=hover_bg, text_color="#FFFFFF", font=("", 16), command=del_city).pack(side="left")
            
            temp_row = ctk.CTkFrame(card, fg_color="transparent")
            temp_row.pack(anchor="w", padx=25, pady=(0, 10))
            ctk.CTkLabel(temp_row, text=city["t"], font=(self.font_family, 56, "bold"), text_color="#FFFFFF").pack(side="left")
            ctk.CTkLabel(temp_row, text=city["desc"], font=(self.font_family, 18), text_color="#FFFFFF").pack(side="left", padx=15, pady=(15,0), anchor="s")
            ...
            m_data = [("🌡", "Feels Like", city.get("feels_like", "--")), 
            ...
            for m_idx, (m_i, m_t, m_v) in enumerate(m_data):
                ...
                ctk.CTkLabel(m_frame, text=m_i, font=(self.font_family, 18), text_color="#FFFFFF").pack(anchor="w", padx=15, pady=(6, 0))
                ctk.CTkLabel(m_frame, text=m_t, font=(self.font_family, 11), text_color="#D1D5DB").pack(anchor="w", padx=15, pady=(0, 0))
                ctk.CTkLabel(m_frame, text=m_v, font=(self.font_family, 15, "bold"), text_color="#FFFFFF").pack(anchor="w", padx=15, pady=(0, 0))
    ...
    def build_setting_row(self, parent, title_key, desc_key, widget_type, state="normal", icon=None):
        ...
        if icon:
            ...
            ctk.CTkLabel(icon_box, text=icon, text_color="#FFFFFF", font=("", 14)).place(relx=0.5, rely=0.5, anchor="center")
            
        text_inner = ctk.CTkFrame(text_col, fg_color="transparent")
        text_inner.pack(side="left")
        
        title_color = "#14142B" if state == "normal" else "#A0AEC0"
        title_lbl = ctk.CTkLabel(text_inner, font=(self.font_family, 14, "bold"), text_color=title_color)
        ...
        if widget_type == "switch":
            ...
            c_btn = ctk.CTkButton(btn_box, text="°C", width=40, height=30, corner_radius=8, font=(self.font_family, 12, "bold"),
            ...
            f_btn = ctk.CTkButton(btn_box, text="°F", width=40, height=30, corner_radius=8, font=(self.font_family, 12, "bold"),
                                  fg_color="#563BFE" if self.temp_unit=="F" else "transparent", 
                                  text_color="#FFFFFF" if self.temp_unit=="F" else "#6E7191",
                                  hover_color="#4A2EE0" if self.temp_unit=="F" else "#E2E8F0")
            ...
            def t_c():
                ...
                if hasattr(self, 'current_city'): self.fetch_weather(self.current_city)
            def t_f():
                self.temp_unit = "F"
            ...
            c_btn.configure(command=t_c)
            f_btn.configure(command=t_f)
